...
def get_activations(files, model, batch_size=50, dims=2048,
    ...
    pred_arr = np.empty((n_used_imgs, 128))
    ...
    for i in range(0, n_used_imgs, args.batch_size):
        ...
        images = np.array([np.load(str(f)).astype(np.float32)
        ...
        batch = torch.from_numpy(images).type(torch.FloatTensor).unsqueeze(1)
        if cuda:
            batch = batch.cuda()
