...
def train(generator, discriminator, autoencoder, g_optim, d_optim, step, iteration=0, startpoint=0, used_sample=0,
    ...
    origin_loader = gain_sample(batch_size[step], (800, 256))
    data_loader = iter(origin_loader)
    ...
    while used_sample < n_sample_total:
        ...
        real_predict = discriminate(discriminator, real_image, std, args.n_gpu)
        real_loss = nn.functional.softplus(-real_predict).mean()



        fake_image = generate(generator, args.n_gpu)
        fake_predict = discriminate(discriminator, fake_image, std, args.n_gpu)
        ...
        progress_bar.set_description(
            (f'D_Loss: {d_losses[-1]:.4f}  G_Loss: {g_losses[-1]:.4f}  Alpha: {alpha:.4f}')
        )


# Create models
generator = DC_Generator(n_fc, dim_latent, dim_input).to(device)
discriminator = DC_Discriminator(sr=args.spectral_reg, instance_noise=args.instance_noise).to(device)
